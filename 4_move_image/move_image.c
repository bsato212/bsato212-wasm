...
struct context {
  ...
  /**
   * Rectangle that the paddle texture will be rendered into
   */
  SDL_Rect dest;
...
void loop_handler(void *arg) {
  ...
  ctx->dest.x += ctx->paddle_vx;
  ctx->dest.y += ctx->paddle_vy;

  SDL_RenderClear(ctx->renderer);
  SDL_RenderCopy(ctx->renderer, ctx->paddle_tex, NULL, &ctx->dest);

  sprintf(text, "X: %d Y: %d", ctx->dest.x, ctx->dest.y);
...
int main() {
  ...
  SDL_CreateWindowAndRenderer(1024, 500, 0, &window, &ctx.renderer);
